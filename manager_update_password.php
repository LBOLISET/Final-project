...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($new_password) && !empty($confirm_password) && !empty($username)) {
        ...
        if ($new_password !== $confirm_password) {
            ...
            echo "<script>window.location.href = 'manager_update_password.html';</script>";
            exit();
        }

        // Update the password in the database
        $sql = "UPDATE managers SET password = '$new_password' WHERE username = '$username'";
    ...
    header("Location: manager_update_password.html"); // Redirect if accessed directly
}
?>
